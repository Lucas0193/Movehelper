...
def myapply():
    page = request.args.get('page', 1, type=int)
    per_page = current_app.config['MOVEHELPER_TASKS_PER_PAGE']
    pagination = UserTasks.query.join(TaskOrders).filter(or_(TaskOrders.manpower1==current_user.id, TaskOrders.manpower2==current_user.id, TaskOrders.manpower3==current_user.id)).with_entities(TaskOrders.id, TaskOrders.task_id, UserTasks.title, TaskOrders.createtime).order_by(TaskOrders.createtime.desc()).paginate(page, per_page=per_page)
